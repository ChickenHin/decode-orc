...
* **Disc Mapper** — analyses the incoming frame stream for tracking errors (skipped, repeated, or missing frames caused by LaserDisc player seek problems) and writes a corrected `ranges` specification back to the stage. Where a disc picture was captured more than once, the best copy is kept: candidates are scored on colour-burst level, white-flag SNR, and black-level PSNR, and the analysis report itemises each duplicate group with the readings behind the choice. Works for both CAV and CLV discs; the analysis can be cancelled from the tool dialog. LaserDisc sources only.
