...
**What it looks like:**
The Teletext Pages dialog (preview window, Observers menu) lists every page it has seen — each with the frame it was last seen at — and renders the selected one. Pages are recovered from a trailing window of frames ending at the current frame, and the list accumulates as you move through the recording, so stepping or playing forward builds up the service's page set. Jumping to an unrelated position restarts the list from the frames preceding the new position. Status messages appear in the status bar along the bottom of the dialog. The Teletext Sink stage exports the same packets as a `.t42` stream.
