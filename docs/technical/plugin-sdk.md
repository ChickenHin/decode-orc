...
| 13 | 4 | Teletext sub-pages: `TeletextCataloguedPage` in `<orc/stage/analysis_sink_results.h>` replaces its single `TeletextPageSnapshot page` member with a `std::vector` of the new `TeletextCataloguedSubPage` (sub-code, first/last seen frame, times seen, page snapshot), ascending by sub-code and never empty. A page number can carry a sequence of sub-pages the service cycles through — the multi-page set of ETSI EN 300 706 Annex A.1 — and cataloguing one assembly per page number kept only whichever sub-page was transmitted last. The page-level `times_seen`, `first_seen_frame` and `last_seen_frame` now cover the whole set. Layout- and source-breaking for any plugin or host reading the teletext catalogue. |
