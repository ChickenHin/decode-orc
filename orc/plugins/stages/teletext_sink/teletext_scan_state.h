/*
 * File:        teletext_scan_state.h
 * Module:      orc-stage-plugin-teletext_sink
 * Purpose:     What a pass over a recording learns about where its teletext
...
// Widest 0-based field line either service's window can reach, plus headroom
// for a caller that widened it: the trackers are indexed by field line, so
// this sizes their tables. Lines past it are simply not tracked.
constexpr int32_t kTeletextMaxTrackedFieldLine = 31;

// Fields per frame, which is what the line mask is kept per.
constexpr size_t kTeletextFields = 2;

// Field lines the mask tables hold.
constexpr size_t kTeletextTrackedLines =
    static_cast<size_t>(kTeletextMaxTrackedFieldLine) + 1;

/// Whether each (field, 0-based field line) has ever carried a packet.
using TeletextLineMask =
    std::array<std::array<bool, kTeletextTrackedLines>, kTeletextFields>;

/// When the line mask stands aside and the full window is read anyway.
struct TeletextLineTrackerOptions {
...
struct TeletextScanSnapshot {
  ...
  TeletextLineMask line_alive{};
  TeletextLineTrackerOptions line_options;
  ...
  bool should_probe(uint64_t frame_index, size_t field_idx,
    ...
    if (field_idx >= kTeletextFields || field_line < 0 ||
 ...
 *
 * Every teletext line of a time-base-corrected recording begins at essentially
 * the same sample, because the position of the data burst in the line is fixed
 * by the standard and the TBC has already removed the timing variation the
 * transport added. The slicer nevertheless sweeps the whole ETSI EN 300 706
 * §6.3 window on every line, because a slicer looking at one line in isolation
...
class TeletextPhaseTracker {
 public:
  ...
  // starts within the first frames of a recording that carries teletext on
  // several lines per field.
  ...
  // Smallest half-width a hint may claim, in samples. Roughly a bit period at
  // either 4FSC sample rate (2,56 samples on PAL, 2,5 on NTSC), so a lock that
  // has drifted by a bit is still inside the pinned window.
  static constexpr double kMinRadiusSamples = 3.0;

  // Half-width above which the locks are too scattered to be a pin: the hint
  // is withheld and the full window swept. At the PAL sample rate the full
  // window is about 71 samples, so this still narrows it fivefold.
...
/**
 * @brief Learns which field lines this recording actually carries teletext on
 *
 ...
 * TeletextScanSnapshot's half of the arrangement.
 */
class TeletextLineTracker {
 public:
  ...
  const TeletextLineMask& alive() const { return alive_; }
 ...
 * the workers produced, and freezes it into a TeletextScanSnapshot for each
 * block of frames they slice. Not thread safe, and not read by the workers:
 * they see only the snapshot.
 */
class TeletextScanState {
 public:
  TeletextScanState(bool pin_phase, bool learn_lines,
                    TeletextLineTrackerOptions line_options = {})
  ...
  TeletextScanSnapshot snapshot() const;
  ...
  const TeletextPhaseTracker& phase() const { return phase_; }
  TeletextLineTracker& lines() { return lines_; }
  const TeletextLineTracker& lines() const { return lines_; }

 private:
  bool pin_phase_ = true;
  bool learn_lines_ = true;
  TeletextLineTrackerOptions line_options_;
  TeletextPhaseTracker phase_;
  TeletextLineTracker lines_;
};

}  // namespace orc

#endif  // ORC_TELETEXT_SCAN_STATE_H
