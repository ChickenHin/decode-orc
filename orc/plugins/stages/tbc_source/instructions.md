...
NTSC-J sources are detected automatically from the metadata signal levels: when the recorded picture black sits at the 0 IRE blanking level instead of the standard 7.5 IRE setup pedestal, the stage reports the non-standard black level to downstream stages (exactly as a CVBS source with an explicit NTSC-J black level would). Fields the decoder marked as padding (`pad` in the TBC metadata, written when the player skipped) are honoured: a frame whose two fields are both padding is flagged as a padding frame, so the host observer pass records "padded, no data" instead of measuring the synthetic content, and padding-aware stages (stacker, TBC Sink) skip it. Colour-sequence phase is never taken from the TBC metadata — stages that need it measure it from the burst via the host colour-frame-phase observer. Associated audio (`.pcm`), EFM disc data (`.efm`), and AC3 RF (`.ac3sym`) sidecars are attached to each frame if the files are present alongside the primary TBC file.
