...
**Syntax repair** presents a damaged page as recovered rather than as transmitted, and is on unless you turn it off.

NAPLPS is a language with a published grammar (ANSI X3.110-1983, FIPS PUB 121), so a page recovered from a damaged recording can be checked against it: an operand byte that has fallen out of the columns numeric data occupies, an opcode that has fallen into them, a coordinate that names a point off the screen, a run of operands with bytes missing from the middle where a packet was lost. Where the recording *independently* says a byte is wrong — it failed the odd parity CEA-516 §3.3 puts on every data byte, or the detector was unsure of it — and the grammar leaves exactly one thing it could have been, the byte is corrected. Where the grammar leaves more than one, nothing is changed and the count of what was left undecided goes in the run's report.

Two things it will not do. It never alters a byte that arrived cleanly, however odd the page looks: a page the sender drew badly is the page the sender drew. And it never fills in bytes that never arrived — a lost packet leaves a hole, and writing a plausible byte into a hole is inventing a page rather than recovering one. What it does instead with a hole is find the operand boundary before it and salvage whatever whole shapes survive on the far side.

Turn it off to read the recording exactly as transmitted. Having the two a click apart is the point: it is how you judge whether a repair recovered the page or merely produced a tidier wrong one.

The viewer says what was done, either way. The message panel at the foot of the window says how widely the pages were altered — "225 of 1604 pages corrected", or that the grammar found nothing to correct, which is a different statement from the pass not having run — and the line above it says what was done to the page in front of you: how many bytes were corrected, how many drawings were trimmed at a gap, how many bytes were left in doubt. The full accounting, down to the record, is written to the log at info level as each catalogue is built, and to the run's report under **NAPLPS lint**.
...
The run's report has a **NAPLPS lint** section giving the totals: how many records were faulted before repair and after, how many bytes were corrected, how many runs resynchronised, and — the number worth reading — how many bytes the recording doubted that the grammar could not decide. That last figure is the honest measure of how much of the service is still guesswork.
