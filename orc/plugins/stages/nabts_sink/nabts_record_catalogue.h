...
class NabtsRecordCatalogue {
 public:
  /**
   * Upper bound on catalogued records. Each entry holds one copy of a record's
   * data, which §8.4.2.5 caps at 1904 bytes, so 1024 entries is under 2 MB.
   *
  ...
  explicit NabtsRecordCatalogue(
      std::size_t max_records = kMaxCataloguedRecords);
  ...
  /// Catalogue contents, ascending by {channel, address, version}.
