...
std::vector<NabtsCataloguedRecord> NabtsRecordCatalogue::records() const {
  ...
  for (const auto& entry : records_) {
    ...
    // settled on. An application record's data is function descriptors
    // (§7.2.2), rendered above, and running it as presentation code would draw
    // nonsense.
    if (nabts_type_is_presentation(record.record_type)) {
      record.page = interpreter_.run(record.data);
    }
  }
  return out;
}
